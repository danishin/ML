...
import engine.{ActualResult, PredictedResult, Query}
import io.prediction.controller.{AverageMetric, EmptyEvaluationInfo, Evaluation, Params}
...
//case class DailyResult(date: LocalDate, tickersToEnter: Seq[String], tickersToExit: Seq[String])
//
...
//
//class BacktestingMetric extends AverageMetric[EmptyEvaluationInfo, Query, PredictedResult, ActualResult] {
...
/**
  * Backtesting Metrics
  *
  * This is the most common method in quantitative equity research.
...
//    val (tickersToEnter, tickersToExit) = prediction.tickerPriceMap
//      .toArray
//      .sortBy(_._2)(Ordering.Double.reverse)
//      .collect {
//        case (ticker, price) if price >= params.enterThreshold => (ticker, true)
//        case (ticker, price) if price <= params.exitThreshold => (ticker, false)
//      }
//      .partition(_._2)
//
//    DailyResult(
//      date = query.date,
//      tickersToEnter = tickersToEnter.map(_._1),
//      tickersToExit = tickersToExit.map(_._1)
//    )
//  }
//
//  def evaluateAll(input: Seq[(EvaluationInfo, Seq[DailyResult])]): BacktestingResult = {
//    val dailyResults = input
//      .flatMap(_._2)
//      .toArray
//      .sortBy(_.date)
//
...
//    var positions = Map[String, Double]()
//    val maxPositions = params.maxPositions
//
//    dailyResults
//
//    for (daily <- dailyResults) {
//      val todayIdx = daily.date
...
//
//      // Determine exit
//      daily.tickersToExit.foreach { ticker => {
